	...
	$customer_order->ID;																		
		...
		//[STATUS] => Active
			...
			if($payment_status=='completed' OR $payment_status=='active'){
				...
				update_user_meta($userId, 'ep_fitness_payment_woo', 'success');
				$user = new WP_User( $userId );
				...
				// Change exprie_date
					...
					if($ep_fitness_package_recurring=='on'){
						...
						$recurring_cycle_count = get_post_meta($package_id, 'ep_fitness_package_initial_expire_interval', true);						
						$recurring_cycle_type = get_post_meta($package_id, 'ep_fitness_package_initial_expire_type', true);
					...
					$periodNum='';
						if($recurring_cycle_count!='' AND $recurring_cycle_type!=''){
							...
							$exprie_date= strtotime ($ep_fitness_exprie_date_old);
							$current_date=strtotime(date('Y-m-d'));
							if( $current_date > $exprie_date){
								$timeToBegin = time() + $periodNum;
							}else{
								$timeToBegin = strtotime($ep_fitness_exprie_date_old) + $periodNum;
							}
