	...
	<div class="col-md-9">
		<?php
			...
			if( $package_name==""){
				...
				$recurring= get_post_meta($package_id, 'ep_fitness_package_recurring', true);
				if($recurring == 'on'){
					$package_amount=get_post_meta($package_id, 'ep_fitness_package_recurring_cost_initial', true).' '.$api_currency.'/'.get_post_meta($package_id, 'ep_fitness_package_recurring_cycle_count', true).' '.__(get_post_meta($package_id, 'ep_fitness_package_recurring_cycle_type', true),'epfitness');
					}else{
					$package_amount=get_post_meta($package_id, 'ep_fitness_package_cost',true).' '.$api_currency;
				}
			}
...
<?php
	...
	<div class="row form-group" id="show_hide_div">
		...
		<div class="col-md-9" >
			<button type="button" onclick="show_coupon();"  class="btn-new btn-custom ctrl-btn"><?php  esc_html_e('Have a coupon?','epfitness');?></button>
		</div>
	</div>
	<?php
