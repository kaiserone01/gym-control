<?php
	...
	$profile=(isset($_REQUEST['fitprofile'])?$_REQUEST['fitprofile']:$f_cpt);
	$paged='1';
	$args = array(
	'post_type' => $profile, 
	'paged' => $paged,
					...
					for($i=1;$i<=$no;$i++){
						...
						if($have_access=='1'){
							...
							if(in_array($i, $day_arr)){
								...
								/*
								$li_data=$li_data.' <h4>'.$done_image.'<a href="'. get_permalink($current_page_permalink).'?&fitprofile=post&id='.$post_content->ID.'&cday='.$i.'">'.substr($post_content->post_title,0,50).'</a></h4>';
								*/
								$arr_params = array( 'fitprofile' => 'post', 'id' => $post_content->ID,'cday'=>$i );
								$li_data=$li_data.' <h4>'.$done_image.'<a href="'.add_query_arg( $arr_params ).'">'.substr($post_content->post_title,0,50).'</a></h4>';
